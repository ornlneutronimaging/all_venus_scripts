...
def run(list_of_autoreduce_path, list_proton_charge):
    ...
    logging.info(f"\t\t{np.shape(sum_counts_over_x) = }")
    logging.info(f"\t\t{list_proton_charge[0] = }")

    axs[0].plot(sum_counts_over_x, label=f"{os.path.basename(list_of_autoreduce_path[0])}, - pc:{list_proton_charge[0]:.2f}")
    axs[0].legend()
    ...
    for run, proton_charge in zip(list_of_autoreduce_path[1:], list_proton_charge[1:]):

        logging.info(f"\tLoading {run}")
        label = f"{os.path.basename(run)} - pc:{proton_charge:.2f}"

       # new_array, o_norm = load_data(run)
        new_array = load_data_mp(run)
        sum_counts_over_y = np.sum(new_array, axis=1)
        sum_counts_over_x = np.sum(sum_counts_over_y, axis=1)
        sum_counts_over_x /= proton_charge
...
if __name__ == "__main__":
    ...
    for _run in list_of_runs:
        ...
        proton_charge = get_proton_charge(nexus_path)/1e12
    ...
    run(list_of_autoreduce_path, list_proton_charge)
