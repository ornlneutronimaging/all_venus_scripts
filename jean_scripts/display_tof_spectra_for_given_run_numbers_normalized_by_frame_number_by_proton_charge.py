...
def run(list_of_autoreduce_path, list_frame_number, list_proton_charge):
    ...
    sum_counts_over_x /= list_frame_number[0]
    sum_counts_over_x /= list_proton_charge[0]
    axs[0].plot(sum_counts_over_x, label=f"{os.path.basename(list_of_autoreduce_path[0])} ({list_frame_number[0]}, {list_proton_charge[0]})")
    axs[0].legend()
    axs[0].set_xlabel("file index")
    axs[0].set_ylabel("Sum counts / frame number / proton_charge")
   
    axs[1].semilogy(sum_counts_over_x, label=f"{os.path.basename(list_of_autoreduce_path[0])}")
    axs[1].set_xlabel("file index")
    axs[1].set_ylabel("Sum counts / frame number / proton_charge")
    plt.show(block=False)
    plt.draw()
    plt.tight_layout()
    plt.pause(0.1)

    for _proton_charge, _frame_number, run in zip(list_proton_charge[1:], list_frame_number[1:], list_of_autoreduce_path[1:]):

        logging.info(f"\tLoading {run}")
        label = f"{os.path.basename(run)} ({_frame_number}, {_proton_charge})"

       # new_array, o_norm = load_data(run)
        new_array = load_data_mp(run)
        sum_counts_over_y = np.sum(new_array, axis=1)
        sum_counts_over_x = np.sum(sum_counts_over_y, axis=1)
        sum_counts_over_x /= _frame_number
        sum_counts_over_x /= _proton_charge
...
if __name__ == "__main__":
    ...
    for _run in list_of_runs:
        ...
        list_frame_number.append(frame_number)
        list_proton_charge.append(proton_charge)

    logging.info(f"{list_of_autoreduce_path =}")

    run(list_of_autoreduce_path, list_frame_number, list_proton_charge)
