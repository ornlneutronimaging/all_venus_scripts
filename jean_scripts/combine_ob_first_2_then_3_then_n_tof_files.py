...
log_file_name = "/SNS/users/j35/log/combine_ob_first_2_then_3_then_n_tof_files.log"
logging.basicConfig(filename=log_file_name,
                    filemode='w',
                    format='[%(levelname)s] - %(asctime)s - %(message)s',
                    level=logging.INFO)
logging.info("*** Starting a new script 'combine_ob_first_2_then_3_then_n_tof_files.py' ***")

top_folder = "/SNS/VENUS/IPTS-33699/shared/"
input_folder = "autoreduce/mcp/"
output_folder = "processed_data/NiPowder/OB_NiPowder_testing_time_binning/"

input_frame_folder = "August17_2024_OB_Second_Frame"
output_frame_folder = "seconf_frame"

full_output_folder = os.path.join(top_folder, output_folder, 'combined', output_frame_folder)

logging.info(f"{top_folder =}")
logging.info(f"{input_folder =}")
logging.info(f"{input_frame_folder =}")
logging.info(f"{output_frame_folder =}")
logging.info(f"{full_output_folder =}")

# get list of runs
_full_input_folder = os.path.join(top_folder, input_folder, input_frame_folder)
