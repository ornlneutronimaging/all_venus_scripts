...
def run(list_of_autoreduce_path, list_frame_number):
    ...
    axs[0].set_ylabel("Sum counts / frame number")
   
    axs[1].semilogy(sum_counts_over_x, label=f"{os.path.basename(list_of_autoreduce_path[0])}")
    axs[1].set_xlabel("file index")
    axs[1].set_ylabel("Sum counts / frame number")
    plt.show(block=False)
    plt.draw()
    plt.tight_layout()
    plt.pause(0.1)

    for _frame_number, run in zip(list_frame_number[1:], list_of_autoreduce_path[1:]):
...
if __name__ == "__main__":
    ...
    run(list_of_autoreduce_path, list_frame_number)
